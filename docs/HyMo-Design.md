...
The GDN block is the linear-attention primitive. The current implementation in [`models/gdn.py:11-80`](../models/gdn.py) is correct in math but uses a Python double-loop over chunks and tokens. HyMo replaces the recurrence with a fused Triton kernel implementing the parallel-chunk algorithm from Yang et al. 2412.06464 (ICLR 2025).
...
- `models/gdn.py` — fla-org fused Triton kernel + per-layer `use_rope` flag + `@torch.compile(mode="reduce-overhead")` on forward (§12a.3)
...
|---|---|---|---|
| 1 | **Fused Triton GDN kernel** | 3-5× GDN path → ~30% wall-clock | `models/gdn.py:chunk_gated_delta_rule` (fla-org) |
...
**The problem:** the current GDN implementation in [`models/gdn.py:11-80`](../models/gdn.py) uses a Python double-loop over chunks and tokens. For T=4096 with chunk_size=64, that's 64 × 4096 = 262,144 Python iterations per GDN forward per micro-batch, × 24 GDN layers × 4 micro-batches = ~25M Python iterations per training step. Each iteration has ~10-20 Python bytecodes (loop, index, attribute access, math). The Python overhead alone is ~5-10× the GPU compute time.

**The solution:** replace the Python double-loop with a single fused Triton kernel implementing the parallel-chunk algorithm from Yang et al. (Gated DeltaNet, arXiv 2412.06464, ICLR 2025). The fla-org `chunk_gated_delta_rule` kernel is the reference implementation; the HyMo kernel is either a vendored copy of the fla-org kernel (zero external deps) or a thin wrapper around the `fla` package (one new dependency).
...
**Implementation location:** `models/gdn.py:forward` (replace Python loop with single kernel call). The kernel is `fla.layers.gated_delta_net.chunk_gated_delta_rule` (or a vendored copy in `models/kernels/chunk_gated_delta_rule.py`).
...
**The problem:** the MoE dispatch in [`models/moe.py`](../models/moe.py) is the second-largest cost after the GDN recurrence. The current dispatch path uses BF16 throughout — including the scatter-add indices, which are integer but stored as BF16 for tensor contiguity. The scatter-add has 2× more bytes than necessary.
