...
> 279 passed / 35 skipped in the default `pytest` run (no `--run-heavy`).
> 316 collected total. The per-phase counts above (308, 321, 342, 325)
> were true at the time of each phase delivery and have drifted as the
> code evolved; the live number is the one that matters today.
...
4. [ ] The architecture doc §12 open questions are resolved. Default values are documented in the doc, so resolution is "accept all defaults" unless you want to override. v1.0 commits to: (1) ~~fla dependency for the GDN kernel~~ **[DONE — superseded by hand-written Triton kernel in `src/hymo/models/gdn_triton.py`; `fla` was never added to dependencies]** , (2) byte-level BPE fallback ON, (3) MLA at position 0 (empirical check at step 1k), (4) start on the full optimization stack (per §12a).
...
6. [x] A clean git working tree on a new branch (e.g., `git checkout -b hymo-v1.0`). → The repo was initialized at `LLM/HyMo/` with `main` as the default branch (the workspace root is not a git repo per `CoreProjects/CLAUDE.md`). Working tree is clean on `main` at commit `f4c64e7`. A feature branch can be created when Phase 2 starts.
...
**Deliverable:** `models/model.py` with the full v1.0 architecture; 32-block forward works; partial-RoPE + NoPE-hybrid works; all init is correct.
...
**What:** Replace the prior Python loop recurrence (`gdn.py:60-80`) with a fused Triton kernel call. Add the `use_rope` per-layer flag (set to `False` for the NoPE-hybrid layers).

**[DONE — superseded by hand-written Triton kernel]** The fla-org package provides `fla.layers.gated_delta_net.chunk_gated_delta_rule`. Add `fla` to `pyproject.toml` dependencies. — **Not what shipped:** the repo ships the hand-written Triton kernel `triton_gated_delta_rule` / `TritonGDNFunction` in `src/hymo/models/gdn_triton.py`; `fla` was never added to dependencies (`pyproject.toml:48` has it commented out).
...
**Why:** The v1 Python recurrence is the v2 throughput bottleneck. The hand-written Triton kernel gives 3-5× speedup, and the NoPE flag enables the position-encoding hybrid in §3.1.
...
**Caveat:** ~~`fla` is a new dependency. If you'd rather implement in-house, the alternative is a hand-written Triton kernel at `models/kernels/gdn.py`. The default is to depend on `fla`.~~ **[DONE — superseded]** The hand-written Triton kernel shipped in `src/hymo/models/gdn_triton.py`; no `fla` dependency was added.
...
**Test:** `tests/test_mqa4_kv_groups.py` — verify `n_kv_groups == 4` and the SDPA `enable_gqa=True` path gives exactly 4 query heads per KV head (the shipped MLA uses a single `F.scaled_dot_product_attention(..., enable_gqa=True)` call — no per-query KV-group lookup table exists in the shipped code). Also `tests/test_partial_rope.py` — verify RoPE is applied only to the first 32 dim of the head.
...
**Files:**
- Modify: `models/model.py:81-160` (rewrite the `HyMo` class)
...
**Files:**
- Modify: `models/model.py:61-78` (the `muP_init` function)
...
```python
# models/model.py — extend muP_init
...
def format_comparison_table(hymo_results: dict) -> str:
    ...
    for metric in ["fineweb_edu_ppl", "hellaswag", "arc_challenge", "mmlu", "gsm8k", "humaneval"]:
        row = f"| {metric} | {hymo_results.get(metric, '?'):.3f} "
...
**Files:**
- New: `docs/hymo-quality-decision-matrix.md`
...
- New: `models/kernels/chunk_gated_delta_rule.py` (if vendoring, else import from `fla`) — **[DONE — superseded]** the hand-written kernel shipped at `src/hymo/models/gdn_triton.py` instead
- Modify: `pyproject.toml` (add `fla` dependency, or note the vendored path) — **[DONE — superseded]** no `fla` dependency was added; `pyproject.toml:48` has it commented out
...
def test_gdn_kernel_matches_reference():
    ...
    # Triton kernel
    from hymo.models.gdn_triton import triton_gated_delta_rule
    y_kernel = triton_gated_delta_rule(v, b, c, g, A)
...
def main():
    ...
    from models.hymo import HyMo  # noqa: E402
...
4. Installs dependencies (PyTorch, lm-eval, etc. — no `fla`; the GDN kernel is the hand-written Triton kernel in `src/hymo/models/gdn_triton.py`).
...
# 4. Resume
ssh root@$(get_pod_ip $POD_ID) "cd /workspace/hymo && \
  torchrun --nproc_per_node=4 training/trainer.py \
    --config configs/hymo_750m.yaml \
    --max-steps 57220 \
    --output-dir /workspace/checkpoints/pretrain \
