...
### `hymo.training` (placeholders for Phase 3)

| Class / function | Purpose |
|---|---|
| `goes_to_adamw`, `goes_to_nor_muon`, `partition_parameters`, `ParameterPartition` | Optimizer partition (claim 2) |
| `NorMuon`, `CautiousAdamW`, `Optimizers`, `build_optimizers` | Dual optimizer |
| `JointWSDScheduler`, `DecaySchedule` | Joint WSD scheduler |
| `wrap_model_with_fsdp`, `fsdp_auto_wrap_policy`, `shard_nor_muon_params`, `RankedParamShard` | FSDP-2 wrapping |
| `save_checkpoint`, `load_checkpoint`, `CheckpointState` | DCP save/load |
| `get_val_batch`, `compute_validation_loss`, `ValMetrics` | Held-out validation |
| `Trainer`, `TrainerConfig`, `train_step_result` | Training loop |
...
support lands with the ablation config in Phase 4).

| Module | What |
|---|---|
| `training/optimizer.py` | `NorMuon.step` (Newton-Schulz orthogonalization, cautious mask, FP32 master); `CautiousAdamW.step` (FP32 master, cautious mask on 2D). |
| `training/scheduler.py` | `JointWSDScheduler.get_factor` (warmup / stable / decay with min_lr_ratio floor). |
| `training/fsdp.py` | `wrap_model_with_fsdp`, `fsdp_auto_wrap_policy` (per-expert wrap), `shard_nor_muon_params` (sort + round-robin, 5% balance). |
| `training/checkpoint.py` | `save_checkpoint` / `load_checkpoint` via DCP. |
| `training/validation.py` | `get_val_batch` (mmap val.bin, deterministic window), `compute_validation_loss` (model.eval + no_grad). |
| `training/trainer.py` | `Trainer.train_step`, `train`, `save`, `load`, `evaluate` (FSDP-aware grad norm, NaN-skip, joint WSD step, MTP loss, val every 2000 steps, ckpt every 4000 steps). |

**Gate:** `pytest tests/unit/test_training.py` + `test_fsdp_trainer.py` pass;
a 100-step training probe on 1 GPU shows loss decreasing; checkpoint save/load
round-trips.

### Phase 4: Data Pipeline + Eval + Ablations
...
**Total deliverable:** 47 Python files, 308 tests (all passing),
mypy --strict clean, ruff clean, complete public API, no algorithmic
logic — exactly the Phase 1 scope per the user's request.
